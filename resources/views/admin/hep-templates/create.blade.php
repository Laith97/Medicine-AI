...
@section('title', 'Create HEP Template')
...
<div class="container-fluid px-4 py-4">
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4" style="background:linear-gradient(135deg,#1e293b 0%,#334155 100%);border-radius:16px;padding:1.4rem 1.6rem;box-shadow:0 8px 24px rgba(15,23,42,0.12)">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h1 style="font-size:1.35rem;font-weight:800;color:#fff;letter-spacing:-0.02em;margin:0">Create HEP Template</h1>
                <p style="font-size:0.78rem;color:rgba(255,255,255,0.75);margin:2px 0 0">Build a pre-configured rehab program for your clinicians</p>
            </div>
        </div>
        <a href="{{ route('admin.hep-templates.index') }}" class="btn btn-sm" style="background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.18);color:#fff;border-radius:10px;font-weight:700"><i class="fas fa-arrow-left me-1"></i> Back to Templates</a>
    </div>

    <form method="POST" action="{{ route('admin.hep-templates.store') }}" id="templateForm">
        @csrf
        <div class="row g-4">
            <div class="col-lg-8">
                <!-- Basic Info -->
                    ...
                    <div class="card-body p-4">
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <input type="text" name="name" value="{{ old('name') }}" required class="form-control @error('name') is-invalid @enderror" placeholder="e.g. Post-ACL Phase 1" style="border-radius:10px;height:42px;border:1px solid #e2e8f0">
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="description" rows="3" required class="form-control @error('description') is-invalid @enderror" placeholder="Describe the program goals and population..." style="border-radius:12px;border:1px solid #e2e8f0">{{ old('description') }}</textarea>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="category" required class="form-select @error('category') is-invalid @enderror" style="border-radius:10px;height:42px;border:1px solid #e2e8f0">
                                    <option value="">Select Category</option>
                                    @foreach($categories as $category)<option value="{{ $category }}" {{ old('category')==$category?'selected':'' }}>{{ ucfirst(str_replace('_',' ',$category)) }}</option>@endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="diagnosis_type" class="form-select @error('diagnosis_type') is-invalid @enderror" style="border-radius:10px;height:42px;border:1px solid #e2e8f0">
                                    <option value="">General / All</option>
                                    @foreach($diagnosisTypes as $type)<option value="{{ $type }}" {{ old('diagnosis_type')==$type?'selected':'' }}>{{ ucfirst(str_replace('_',' ',$type)) }}</option>@endforeach
                                </select>
                                @error('diagnosis_type')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                            <div class="col-md-6">
                                ...
                                <input type="number" name="duration_weeks" value="{{ old('duration_weeks',4) }}" min="1" max="52" required class="form-control" style="border-radius:10px;height:42px;border:1px solid #e2e8f0">
                            </div>
                            <div class="col-md-6">
                                ...
                                <input type="number" name="frequency_per_week" value="{{ old('frequency_per_week',3) }}" min="1" max="7" required class="form-control" style="border-radius:10px;height:42px;border:1px solid #e2e8f0">
                            </div>
                            <div class="col-md-6">
                                <div class="form-check form-switch mt-3">
                                    <input class="form-check-input" type="checkbox" name="is_active" value="1" id="is_active" {{ old('is_active',true)?'checked':'' }} style="width:44px;height:24px">
                                    <label class="form-check-label fw-bold" for="is_active" style="font-size:.78rem;color:#334155;text-transform:uppercase;letter-spacing:.04em;margin-left:8px">Active (visible to clinicians)</label>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card border-0 shadow-sm mb-4" style="border-radius:16px;overflow:hidden">
                    ...
                    <div class="card-body p-4">
                        <div class="mb-3">
                            ...
                            <div id="goals-hidden" style="display:none;">@if(old('goals')) @foreach(old('goals') as $goal)<input type="hidden" name="goals[]" value="{{ $goal }}">@endforeach @endif</div>
                        </div>
                        <div class="mb-1">
                            ...
                            <div id="precautions-hidden" style="display:none;">@if(old('precautions')) @foreach(old('precautions') as $prec)<input type="hidden" name="precautions[]" value="{{ $prec }}">@endforeach @endif</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card border-0 shadow-sm mb-4" style="border-radius:16px;overflow:hidden">
                    <div class="card-header bg-white d-flex justify-content-between align-items-center" style="padding:1rem 1.25rem;border-bottom:1px solid #eef2f7">
                        ...
                        <small style="font-size:.72rem;color:#64748b">Required · at least 1</small>
                    </div>
                    <div class="card-body p-4">
                        <!-- Add Row -->
                            ...
                            <div class="row g-2 align-items-end">
                                <div class="col-md-5">
                                    ...
                                    <select id="ex_select" class="form-select" style="border-radius:10px;height:38px;border:1px solid #e2e8f0;font-size:.88rem">
                                        ...
                                        @foreach($exercises as $ex)<option value="{{ $ex->id }}" data-name="{{ $ex->name }}" data-category="{{ $ex->category }}">{{ $ex->name }} — {{ ucfirst($ex->category) }}</option>@endforeach
                                    </select>
                                </div>
                                <div class="col-md-2">
                        ...
                        <div class="table-responsive" style="border:1px solid #e2e8f0;border-radius:12px;overflow:hidden">
                            <table class="table mb-0" style="font-size:.84rem" id="exercisesTable">
                                ...
                                <tbody id="exercisesBody"><tr id="emptyRow"><td colspan="6" class="text-center py-4" style="color:#94a3b8;font-size:.88rem">No exercises added yet — add at least one above</td></tr></tbody>
                            </table>
                        </div>
                        <div id="exercisesHidden"></div>
                        @error('exercises')<div class="text-danger small mt-2">{{ $message }}</div>@enderror
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-4">
                <div class="card border-0 shadow-sm" style="border-radius:16px;position:sticky;top:24px">
                    <div class="card-body p-4">
                        <h6 style="font-weight:800;color:#0f172a;font-size:.88rem;margin-bottom:12px">Publish</h6>
                        <p style="font-size:.78rem;color:#64748b">Template will be available to clinicians when active. You can keep it inactive while drafting.</p>
                        <div class="d-grid gap-2 mt-3">
                            <button type="submit" class="btn text-white" style="background:#0f172a;border-radius:12px;font-weight:700;padding:11px"><i class="fas fa-save me-1"></i> Create Template</button>
                            <a href="{{ route('admin.hep-templates.index') }}" class="btn btn-light border" style="border-radius:12px;font-weight:600;padding:11px">Cancel</a>
                        </div>
                    </div>
                </div>

                <div class="card border-0 shadow-sm mt-3" style="border-radius:16px;background:#f8fafc;border:1px solid #e2e8f0 !important">
                    <div class="card-body p-3">
                        <div style="font-size:.78rem;font-weight:700;color:#334155;display:flex;align-items:center;gap:8px"><i class="fas fa-lightbulb" style="color:#d97706"></i> Tips</div>
                        <ul style="font-size:.76rem;color:#64748b;margin:8px 0 0;padding-left:18px">
                            <li>Use week numbers to progress intensity</li>
                            <li>Balance strength / flexibility / balance</li>
                            <li>Add clear progression notes</li>
                        </ul>
                    </div>
                </div>
            </div>
...
function addExercise(){
    ...
    const exName=sel.options[sel.selectedIndex].dataset.name;
    const week=document.getElementById('ex_week').value||1;
    const sets=document.getElementById('ex_sets').value;
    const reps=document.getElementById('ex_reps').value;
    const duration=document.getElementById('ex_duration').value;
    const rest=document.getElementById('ex_rest').value;
    const freq=document.getElementById('ex_freq').value;
    ...
    const fields={exercise_id:exId,week_number:week,sets, reps, duration_seconds:duration, rest_seconds:rest, frequency:freq, progression_notes:notes, order:exIndex};
...
function removeExercise(btn, idx){
    ...
    if(!document.getElementById('exercisesBody').children.length){
        document.getElementById('exercisesBody').innerHTML='<tr id="emptyRow"><td colspan="6" class="text-center py-4" style="color:#94a3b8;font-size:.88rem">No exercises added yet — add at least one above</td></tr>';
    }
    updateCount();
}
function updateCount(){
    const count=document.querySelectorAll('#exercisesHidden input[name*="[exercise_id]"]').length;
    document.getElementById('exerciseCount').textContent=count+' added';
    document.getElementById('exerciseCount').style.background=count?'#ecfdf5':'#f1f5f9';
    document.getElementById('exerciseCount').style.color=count?'#065f46':'#64748b';
    document.getElementById('exerciseCount').style.borderColor=count?'#a7f3d0':'#e2e8f0';
}
document.getElementById('templateForm')?.addEventListener('submit', function(e){
    const count=document.querySelectorAll('#exercisesHidden input[name*="[exercise_id]"]').length;
    if(count===0){ e.preventDefault(); alert('Add at least one exercise to the template'); }
});
</script>
@endpush
